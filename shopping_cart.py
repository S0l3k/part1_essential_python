...
#Создать ф-цию, которая предлагает действия с корзиной "Добавить товар", "Удалить товар", 
# "Закрыть корзину"

shopping_cart = []

def delete_item_in_shopping_cart(cart: list) -> list:
    """Выводит список товаров в корзине. Получает название товара, 
    который необходимо удалить из корзины."""
...
def action_with_shopping_cart(action: int) -> None:
    ...
    if action == 1:
        ...
        while stop:
            item = str(input('Введите название товара, который хотите добавить в корзину '
            '(Если закончили добавлять товар, введите "1"): '))
            if item == '1':
                stop = False
            else:
